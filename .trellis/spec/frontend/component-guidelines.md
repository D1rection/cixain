...
- **断点**：≤768px 折叠为汉堡按钮 + 全宽 6 列导航菜单（入口只显示图标，文字保留在 `aria-label` 等可访问名称中；`.actions` 用 `display: contents` 并入菜单行；移动端隐藏 GitHub、背景，显示「文章索引」）；≤480px 时 `.inner` padding 收窄为 12px。移动端一级入口数量固定为 6，不随分类/系列/标签数量增长。
- **移动端文章索引**：`/browse/` 是分类、系列、标签的统一入口，面向用户的页面名称为「文章索引」；聚合统计必须与 `Sidebar` 共享确定性数据源，不在 `NavBar` 内嵌完整标签云。
- **文章索引视觉基线**：`Browse` 必须以 `Archive` 为同类页面基准，沿用 680px 内容宽度、页面四周留白、小号像素标题、细分隔线、4px 小圆角和轻量 hover 底色。分类、系列、标签应使用无卡片的紧凑文字目录；外部设计参考只能补充排版方法，不得引入与归档页脱节的大号展示标题、满宽贴边布局、卡片或胶囊标签。移动端链接保持至少 44px 可点击高度。
