...
- **断点**：≤768px 折叠为汉堡按钮 + 全宽 6 列导航菜单（入口包含图标与短文字标签，`.actions` 用 `display: contents` 并入菜单行；移动端隐藏 GitHub、背景，显示「浏览」）；≤480px 时 `.inner` padding 收窄为 12px。移动端一级入口数量固定为 6，不随分类/系列/标签数量增长。
- **移动端内容索引**：`/browse/` 是分类、系列、标签的统一入口；聚合统计必须与 `Sidebar` 共享确定性数据源，不在 `NavBar` 内嵌完整标签云。
